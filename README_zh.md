...
- `decompile(version=None, *args, **kw)`：将代码对象反编译为源代码。（已弃用）
...
>>> def f():print("Hello")
...
>>> c=Code.fromfunc(f) # 或c=Code(f.__code__)
...
2026-7-5(v1.3.5):增加了3.13+的`FrameLocalsProxy`的支持，和非windows平台的高dpi支持。改进了`pyobject.search`的算法。  
